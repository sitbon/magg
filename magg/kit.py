...
class KitConfig(BaseSettings):
    ...
    servers: dict[str, ServerConfig] = Field(default_factory=dict, description="Servers included in this kit")
...
class KitManager:
        ...
        for kitd_path in self.kitd_paths:
            ...
            for file_path in kitd_path.glob("*.json"):
                if file_path.is_file():
                    ...
                    if kit_name in kits:
                        logger.warning("Duplicate kit %r found at %s, keeping %s", kit_name, file_path, kits[kit_name])
                    else:
    ...
    def load_kit_to_config(self, kit_name: str, config: "MaggConfig") -> tuple[bool, str]:
        ...
        config.kits[kit_name] = KitInfo(
            name=kit_name, description=kit_config.description, path=str(kit_path), source="file"
        )
