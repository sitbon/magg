...
class TestAddServer:
    ...
    def temp_dirs(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            yield {"tmpdir": tmpdir, "project_root": project_root, "cwd": cwd, "subdir": subdir}
    ...
    async def test_add_server_duplicate_name(self, magg_server):
        ...
        with patch.object(magg_server.server_manager, "mount_server", new_callable=AsyncMock) as mock_mount:
            ...
            result1 = await magg_server.add_server(
                name="existingserver", source="https://github.com/example/test1", command="python old.py"
            )
            assert result1.is_success

            # Try to add duplicate
            result2 = await magg_server.add_server(
                name="existingserver", source="https://github.com/example/test2", command="python new.py"
            )
    ...
    async def test_add_server_invalid_name_accepted(self, magg_server):
        ...
        result = await magg_server.add_server(
            name="123-invalid", source="https://github.com/example/test", command="python server.py"
        )
    ...
    async def test_add_server_http_no_command(self, magg_server):
        ...
        with patch.object(magg_server.server_manager, "mount_server", new_callable=AsyncMock) as mock_mount:
            ...
            result = await magg_server.add_server(
                name="httpserver", source="https://github.com/example/test", uri="http://localhost:8080"
            )
    ...
    async def test_add_server_mount_failure(self, magg_server):
        ...
        with patch.object(magg_server.server_manager, "mount_server", new_callable=AsyncMock) as mock_mount:
            ...
            result = await magg_server.add_server(
                name="failserver", source="https://github.com/example/test", command="python server.py"
            )
